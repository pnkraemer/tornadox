...
def ek0_diagonal_terminal_value(*, num_derivatives=5):
    """The diagonal EK0, but only for computing the terminal value.

    Uses adaptive steps and proportional control.
    Uses time-varying, vector-valued diffusion.
    ...
    DiagonalEK0State = namedtuple(
        "DiagonalEK0State", ("u", "dt_proposed", "error_norm", "stats")
    )
    ...
    def perform_step_fn(
        ...
        *,
        ...
        def body_fun(s):
            ...
            # Attempt step
            u_new, _, error = ek0_diagonal.attempt_step_forward_only(
                f=f,
                m=m0,
                c_sqrtm=c_sqrtm0,
                p=p,
            ...
            error = dt_clipped * error  # The EK0-step does not do this.

            # Normalise the error
            m_new, _ = u_new
            u1_ref = jnp.maximum(jnp.abs(m_new[0, :]), jnp.abs(m0[0, :]))
            ...
            s_new = DiagonalEK0State(
                u=u_new, dt_proposed=dt_new, error_norm=error_norm, stats=stats
            )
            return t_new, s_new

        larger_than_1 = 1.1
        init_val = DiagonalEK0State(
            u=state0.u,
